...
struct EventDispatcher{
    std::unordered_map<uint64_t, std::function<void(Manager&, EntityId)>> eventMap;

    template<typename... Tags>
    uint64_t registerEvent(){
        constexpr uint64_t hash = hashTags<Tags...>();

        eventMap[hash] = [](Manager& manager, EntityId entity){
    ...
    void dispatchConstruction(Manager& manager, EntityId entity, uint64_t hash){
        auto it = eventMap.find(hash);
        if(it != eventMap.end())
            it->second(manager, entity);
    }
};
