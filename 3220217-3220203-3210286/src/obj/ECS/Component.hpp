...
enum struct StatType : uint8_t { TIMED, PERMANENT };
...
struct FireballAbilityComponent {
    uint8_t count = 0;
};
struct FireballItemTag {};

struct ElectricAbilityComponent {
    uint8_t count = 0;
};
struct ElectricItemTag {};

struct ElectricTrailComponent {
    float life;
    float maxLife;
    float damage;
    float damageTimer;
...
struct DifficultyComponent {
    ...
    float attributeSpawnTimer = 0.0f;

    float enemySpawnInterval = 3.0f;
    float attributeSpawnInterval = 5.0f;
...
struct HeavyAttackingStateTag{};
struct DamagedStateTag{};
struct DashingStateTag{};
...
struct PngComponent{
    ...
    PngComponent(std::string _filepath, float _scale = 1.0f) : scale(_scale) {
        texture = LoadTexture(_filepath.c_str());
    }
};
